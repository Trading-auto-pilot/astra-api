 ...
 *
 * @param {object} opts
 * @param {object} opts.service   - service adapter con getLogLevel, setLogLevel, getInfo, updateCommunicationChannel
...
module.exports = function buildStatusRouter({ service, logger, moduleName }) {
  ...
  router.put("/communicationChannels", async (req, res) => {
    ...
    try {
      ...
      // delega la vera applicazione al service adapter
  ...
  router.get("/logLevel", (_req, res) => {
    const current =
      service && typeof service.getLogLevel === "function"
        ? service.getLogLevel()
        : null;
    res.status(200).json({ dbmanager: current });
  });
  ...
  router.put("/logLevel", (req, res) => {
    ...
    const current =
      service && typeof service.getLogLevel === "function"
        ? service.getLogLevel()
        : logLevel;
    res.status(200).json({ success: true, dbmanager: current });
  });
