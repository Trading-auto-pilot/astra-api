---
sidebar_position: 5
title: Verifica tickerScanner — Comportamento con gli ETF
---

# Verifica tickerScanner — Comportamento con gli ETF

## Navigazione

1. Verifica presenza ETF nel flusso di scansione
2. Punto da verificare
3. Domande aperte
4. Fasi del flusso da controllare
5. Priorità
6. Proposta architetturale — Fase 1: Separazione ETF, Fase 2: Pipe aggiuntive

## Verifica presenza ETF nel flusso di scansione

## Punto da verificare

Verificare se gli ETF presenti nell'universo di input arrivano effettivamente in fondo al processo del `tickerScanner` oppure vengono eliminati o filtrati in una fase intermedia.

## Domande aperte

- Gli ETF entrano nel processo di scansione (universe) insieme ai titoli azionari ordinari?
- Esiste un filtro esplicito per tipo strumento (`ETF`, `stock`, `fund`) in qualche fase del pipeline?
- Se vengono filtrati, in quale punto esatto del flusso (pre-scan, durante scoring, post-scan)?
- Il filtraggio è intenzionale (ETF non adatti alla strategia) o è un effetto collaterale non voluto?
- Gli ETF che superano il processo vengono poi gestiti correttamente da `decision-engine`, `capital-manager` e `brokerExecutor-ibkr`?

## Fasi del flusso da controllare

1. Costruzione universe — la lista iniziale dei ticker include ETF? Verificare la fonte dati (FMP screener, lista manuale, DB) e se esiste un filtro `type = stock` che esclude gli ETF a monte.
2. Fase di scoring / ranking — i criteri di scoring (volume, ATR, trend) penalizzano strutturalmente gli ETF rispetto ai titoli singoli?
3. Fase di selezione finale — esiste una soglia o un criterio che di fatto esclude gli ETF (es. bassa volatilità, volume non sufficiente)?
4. Integrazione con cacheManager — gli ETF vengono richiesti correttamente al provider storico? FMP e Alpaca richiedono endpoint diversi per ETF vs stock in alcuni casi.
5. Integrazione con IBKR — la risoluzione del `conid` tramite `ibkr-bridge` funziona correttamente per gli ETF (secType `ETF` vs `STK`)?

## Priorità

Media — da chiarire prima di espandere l'universo di analisi a strumenti misti (stock + ETF). Se gli ETF vengono filtrati silenziosamente, il sistema non darà errori ma produrrà un universo incompleto rispetto alle aspettative.

## Proposta architetturale — Fase 1: Separazione ETF, Fase 2: Pipe aggiuntive

## Obiettivo

Evolvere il sistema da una pipeline unica mista (stock + ETF concorrenti con stessi criteri) a pipeline parallele per asset class, con composizione configurabile della pipe live finale. L'implementazione è divisa in due fasi per minimizzare il rischio e validare il comportamento ETF prima di aggiungere nuove asset class.

---

## Fase 1 — Separazione ETF (da implementare per prima)

### Problema attuale

Stock ed ETF competono nel ranking con criteri pensati esclusivamente per le aziende. Un ETF ha strutturalmente `valuation_score = null` e `quality_score = null` perché le metriche fondamentali (PE, PB, ROE, Altman Z) non esistono per uno strumento che non è un'azienda. Questo non indica un ETF scadente — indica che il criterio di valutazione è sbagliato per quella asset class.

Risultato: gli ETF partono svantaggiati strutturalmente nel ranking misto e tendono a non raggiungere la pipe live non perché siano cattivi candidati ma perché vengono valutati con lo strumento sbagliato.

### Analisi del codice attuale

Dal codice sorgente analizzato, i punti critici sono:

`screenerService.js` — `TICKSCANNER_SCREENER_FILTERS` nel DB potrebbe contenere `isEtf=false` o `type=stock`, escludendo gli ETF già a monte prima di entrare nel processo. Verificare il valore attuale di questo setting.

`scoringService.scoreSymbol` — esiste già una branch ETF ma minimale: `valuation` e `quality` sono `EMPTY`, il `total_score` è calcolato solo con `{ risk: 0.5, momentum: 0.5 }`. Se il `momentumScore` è null (per dati storici mancanti), il `total_score` dipende solo dal beta.

`rankingDailyService` — bucket `ETF_GENERAL` già esistente con limite di 30. Il filtro `exclude_etf` è supportato esplicitamente. Verificare se viene passato nei job scheduler.

`cacheManager._ibkrResolveConid` — `secType: "STK"` fisso. Gli ETF su IBKR richiedono `secType: "ETF"` → fallback silenzioso a FMP/Alpaca → possibile `momentumScore = null`.

Filtri utente — se l'utente ha filtri su `quality_score > X` o `valuation_score > X`, tutti gli ETF vengono eliminati silenziosamente (i loro score sono sempre null).

### Modifiche per la Fase 1

1. Aggiungere `asset_class` alla tabella `universe`

```
ALTER TABLE universe ADD COLUMN asset_class VARCHAR(20) DEFAULT 'STOCK';
```

Valorizzato in `universeService.buildUniverseRecord`:

```
asset_class: profile.isEtf || profile.isFund ? "ETF" : "STOCK"
```

2. Fix `secType` in cacheManager

```
const SEC_TYPE_MAP = { STOCK: "STK", ETF: "ETF" };
const secType = SEC_TYPE_MAP[asset_class] || "STK";
```

3. Scoring profile separato per ETF

Metriche significative per un ETF: momentum tecnico (prezzo vs SMA, trend), volatilità (ATR%), volume medio, beta vs indice benchmark, correlazione con il mercato.

```
const SCORING_PROFILES = {
  STOCK: {
    weights: { valuation: 0.3, quality: 0.4, risk: 0.2, momentum: 0.1 },
  },
  ETF: {
    weights: { momentum: 0.6, risk: 0.4 },
    // risk calcolato solo da beta e ATR%, non da Altman Z o Debt/Equity
  },
};
```

4. Pipe live configurabile — top N stock + top M ETF

`AST_RANKING_DAILY` è già la fonte della pipe live (pipe virtuale `pipe_id=0`). La composizione diventa un parametro:

```
const LIVE_PIPE_COMPOSITION = {
  LARGECAP:    20,
  MIDCAP:      10,
  SMALLCAP:     5,
  ETF_GENERAL: 10,  // top M ETF, configurabile
};
```

Il `decision-engine` riceve simboli senza sapere di che asset class sono — i pattern tecnici (breakout, zone di supporto) funzionano allo stesso modo su SPY o su NVDA. La differenza è nei parametri (ATR più largo per ETF molto diversificati) gestibile con un moltiplicatore per asset class.

### Cose da verificare prima di iniziare la Fase 1

- Valore attuale di `TICKSCANNER_SCREENER_FILTERS` nel DB — contiene filtri che escludono gli ETF?
- Il job scheduler che avvia `rankingDailyService` passa `filters: { exclude_etf: true }`?
- I filtri utente attivi hanno soglie su `quality_score` o `valuation_score` che eliminano tutti gli ETF?
- Il provider storico attivo è FMP, ALPACA o IBKR? (IBKR richiede il fix `secType` con priorità alta)

---

## Fase 2 — Pipe aggiuntive: Metalli, Futures e altre asset class

Da implementare solo dopo aver validato la separazione ETF in produzione.

### Principio di design

Ogni nuova asset class è additiva: aggiunge un file sorgente, una entry nel registry degli scoring profile, un bucket nel ranking. Non modifica il codice esistente.

### Sorgenti dati per le nuove pipe

Metalli e futures non esistono nell'endpoint `/stable/company-screener` di FMP. Richiedono sorgenti dedicate:

```
// lib/assetSources/metalSource.js
// Simboli FMP per metalli: GCUSD, SIUSD, PTUSD, HGUSD
// Simboli IBKR: GC (Gold), SI (Silver), secType: "CMDTY"

// lib/assetSources/futureSource.js
// ES (S&P500 Futures), NQ (Nasdaq), CL (WTI Oil), NG (Gas)
// IBKR secType: "FUT", lastTradeDateOrContractMonth richiesto
```

I simboli vengono inseriti direttamente in `universe` con `asset_class = "METAL"` o `asset_class = "FUTURE"`, bypassando lo screener FMP equity.

### Registry scoring profile (esteso)

```
const SCORING_PROFILES = {
  STOCK:  { weights: { valuation: 0.3, quality: 0.4, risk: 0.2, momentum: 0.1 } },
  ETF:    { weights: { momentum: 0.6, risk: 0.4 } },
  METAL:  { weights: { momentum: 0.6, risk: 0.4 } },
  // risk per metalli: solo volatilità (ATR%), nessun beta aziendale
  FUTURE: { weights: { momentum: 0.7, risk: 0.3 } },
  // futures: momentum dominante, breve finestra temporale
};

// Aggiungere CRYPTO domani = aggiungere una entry. Zero modifiche altrove.
```

### secType map estesa in cacheManager
...
### Bucket e composizione pipe live finale
...
// Totale: 55 simboli monitorati live, distribuiti per asset class
```

### Componenti già agnostici (non richiedono modifiche)

- `momentumCalculator` — lavora su qualsiasi serie OHLCV, già compatibile con qualsiasi asset class
- `filterEngine` — filtra su campi numerici, agnostico
- `decision-engine` — riceve simboli da `AST_RANKING_DAILY` senza conoscere l'asset class

### Stima effort Fase 2

| Task | Stima |
| --- | --- |
| `asset_class` in `universe` + `buildUniverseRecord` | 1h |
| Bucket `detectBucket` in `rankingDailyService` | 30min |
| Scoring profile registry | 2h |
| `secType` map in cacheManager | 30min |
| `MetalSource` (sorgente + simboli) | 2-3h |
| `FutureSource` (sorgente + gestione scadenze) | 3-4h |
| `LIVE_PIPE_COMPOSITION` configurabile | 1h |
| Totale | ~10-12h |
...
Stock + ETF + Metalli + Futures in parallelo nella pipe live, ognuno valutato con i criteri corretti per la sua asset class, con N e M configurabili per bucket senza toccare il codice. Aggiungere una quinta asset class (es. crypto o forex) = aggiungere un file sorgente, una entry nel registry e un bucket — nessuna modifica al codice esistente.

---

## TickerScanner — Analisi locale aggiuntiva
...
async function handleRankingDailyView(req, res, { userId, asOfDate, datahubAxios, logger }) {
  ...
  try {
    ...
    // Arricchisce con dati universe (per avere price, sector, country, ecc.)
    // Se la tabella daily_scores contiene già i dati completi, può bastare quella
    // Altrimenti fare join con universe via datahub
...
// (solo se si vuole mantenere il vecchio path di lettura)
...
await runConcurrent(
  ...
  async (record) => {
    const scoreRow = {
      ...
      // pipe_id=0 indica dato di sistema (non user-specific)
      pipe_id: 0,
      user_id: null,
    };
    try {
