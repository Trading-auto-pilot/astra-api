...
class SchemaReader {
   ...
   * @returns {Promise<Array<{name: string, schema: string, type: 'table'|'view', columns: Array, primaryKeys: Array}>>}
   */
  async readSchema() {
    if (!this.pool) {
    ...
    const primaryDatabase = this.config.database;
    const simulDatabase = this.config.simulDatabase;
    const schemas = Array.from(
      new Set(
        [primaryDatabase, simulDatabase]
    ...
    for (const database of schemas) {
      ...
      for (const table of tables) {
        ...
        if (seenTableNames.has(tableName)) {
          this.logger.warning(
            `[schemaReader] Skipping duplicate table/view name=${tableName} from schema=${database} because it already exists in another scanned schema`
          );
          continue;
        }
