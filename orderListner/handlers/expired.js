...
logger.trace(`[FILL] Strategy details : ${JSON.stringify(myStrategy)}`)
  ...
  try{
    ...
    retData.data.operation = 'EXPIRED';
  ...
  try{
    ...
    logger.trace(`[FILL] Invio email richiamando ${alertingUrl}/email/send Ordine cancellato ${data.order.id}`);
    await axios.post(`${alertingUrl}/email/send`, {
        to: 'expovin@gmail.com',
        subject: `Ordine ${data.order.id} Expired`,
        body: `Ordine ${data.order.id} Scaduto. Simbolo ${data.order.symbol} qta ${data.order.qty}`
    });
  } catch (err) {
