...
if [ "$dnsIp" != "$currentIp" ];
 then
		...
		curl -X POST "http://localhost:3008/email/send" \
		-H "Content-Type: application/json" \
		-d '{"to":"expovin@gmail.com", "subject":"Cambio indirizzo ip VM trading-system su GCP", "body":"Indirizzo IP modificato. Nuovo indirizzo $currentIp"}'
fi
if [ "$dnsIp" = "$currentIp" ];
 then
