...
router.put("/bulk", async (req, res) => {
  ...
  /**
   * PUT /fundamentals/touch
   * Aggiorna last_touch_date = NOW() per una lista di simboli (dbHit path)
   * Body: { symbols: ["AAPL", "MSFT", ...] }
   */
  router.put("/touch", async (req, res) => {
    const symbols = req.body?.symbols;
    if (!Array.isArray(symbols) || !symbols.length) {
      return res.status(400).json({ ok: false, error: "Serve { symbols: [...] }" });
    }
    try {
      const result = await dbManager.touchFundamentalsBulk(symbols.map(s => String(s).toUpperCase()));
      return res.json({ ok: true, ...result });
    } catch (err) {
      logger.error("[PUT /fundamentals/touch] Errore:", err?.message || err);
      return res.status(500).json({ ok: false, error: "Errore aggiornamento last_touch_date" });
    }
  });
